...
hansuu = [["断么九", "場風:東", "場風:南", "場風:西", "場風:北", "自風:東", "自風:南", "自風:西", "自風:北", "三元牌:白", "三元牌:發", "三元牌:中", "門前清自摸和", "一盃口", "平和", "立直", "一発", "ドラ", "赤ドラ", "裏ドラ", "北ドラ", "海底撈月", "河底撈魚"], \
          ["混全帯么九", "三色同順", "三色同刻", "三暗刻", "七対子", "一気通貫", "混老頭", "対々和", "三槓子", "小三元", "ダブル立直"], \
          ["混一色", "純全帯么九", "二盃口"], \
          [], \
          [], \
          ["清一色"], \
          ["国士無双", "四暗刻", "大三元", "字一色", "緑一色", "小四喜", "清老頭", "四槓子", "九連宝燈", "天和", "地和"], \
          ["大四喜", "国士十三面待ち", "四暗刻単騎", "大七星", ""]] # 翻数一覧。このからは単体役満以上

dw = ["混全帯么九", "三色同順", "一気通貫", "混一色", "純全帯么九", "清一色"] # 食い下がりする役

translate_hansuu = {0:"零", 1:"一", 2:"二", 3:"三", 4:"四", 5:"五", 6:"六", 7:"七", 8:"八", 9:"九"} # 下のtrans()のところで使う。ただ単に漢字表記にしたかっただけだから実はあんまり要らない。

def trans(n, a=True): # aはn > 7の時のn倍役満表記で"二翻倍役満"とかにならないようにするためにある。
    if n > 7:
        return trans(n % 6, False) + "倍役満"
    if n == 7:
        return "役満"
    if a:
        return translate_hansuu[n] + "翻"
    return translate_hansuu[n]

def fu():
    global han
    han = 0
    for i in yaku:
        for ia in range(len(hansuu)):
            if i in hansuu[ia]:
                if s[3] == -1 and i in dw:
                    print(i + ":喰い下がり" + trans(ia + 1))
                    han += ia
                else:
                    print(i + ":" + trans(ia + 1))
                    han += ia + 1
        ...
        for ia in range(len(p[i])):
            ...
            if y[0] == "t" and i == 3:
                ...
                if pk_[s[3][1]] == i and s[3][0] == y[1]:
                    ...
                    x += 2 * z
#    print("役:" + str(yaku))
    print("\n翻数:" + TransToKanji.TransToKanji(han) + "翻")
    print("符数:" + TransToKanji.TransToKanji(x) + "符")
    x *= 2 ** (han + 2) # 基礎打点計算
    if s[0] == 0: # 親番で和了
        if s[2]: # 自摸和了
            return (math.ceil((x * 2) / 100) * 100)
        else: # 攏和了
            return (math.ceil((x * 6) / 100) * 100)
    else: # 子
        if s[2]: # 自摸和了
            return (math.ceil((x * 2) / 100) * 100, math.ceil(x / 100) * 100)
        else: # 攏和了
            return (math.ceil((x * 4) / 100) * 100)
...
def tanyao():
    if len(p[3]) + len(f[3]) != 0:
        return
    if all(all("s" + str(ia) not in p[i] and "t" + str(ia) not in f[i] for ia in (0, 6)) and all(all(ib + str(ia) not in p[i] for ib in ("k", "t")) and all(ib + str(ia) not in f[i] for ib in ("p", "a", "d", "k")) for ia in (0, 8)) for i in range(3)):
        yaku.append("断么九")
...
def tyanta():
    if any(any("s" + str(ia) in p[i] or "t" + str(ia) in f[i] for ia in range(1, 5)) or any(any(ib + str(ia) in p[i] for ib in ("t", "k")) or any(ib + str(ia) in f[i] for ib in ("p", "a", "d", "k")) for ia in range(1, 8)) for i in range(3)):
        return
    if len(p[3]) + len(f[3]) == 0:
        yaku.append("純全帯么九")
    else:
...
def main(p_, f_=((), (), (), ()), s_=()): # p ==> 手牌, f ==> 副露, s ==> 盤面(シチュ) (場風, 自風, TF(ツモ-ロン), 和了牌, 状態(-1:副露, 0:面前, 1:暗槓あり面前, 2:立直, 3:ダブル立直)) 全部0m方式で
...
if __name__ == "__main__":
    print(main((("s0"), ("s0"), ("s0", "s0", "t8"), ()), ((), (), (), ()), (0, 0, True, "0s", 2)))